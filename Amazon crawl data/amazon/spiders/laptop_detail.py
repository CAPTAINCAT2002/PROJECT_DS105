...
class LaptopLinksProgress:
    def __init__(self, new_progress=False):  # set new_progress=True if want to reset current progress data
    ...
    def create_new_progress(self):
        laptop_overview = pd.read_csv("laptop_overview_data.csv")
...
class LaptopDetail(scrapy.Spider):
    name = "laptop_detail"
    allowed_domains = ["amazon.com"]
    progress=LaptopLinksProgress(new_progress=False)
    start_urls = [progress.get_first_undone_link()]
